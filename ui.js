...
function allMenu() {
	...
	let mainVisualStatus = true;
	$('.slideAll').on('click', function(){
		if (mainVisualStatus) {
			let slideObj = $('.allvisual ul');
			let slideHtml = VisualSlideList.replace(/class="swiper-lazy" data-/gi,'');
			slideObj.append(slideHtml);
			slideObj.find('li').removeAttr('style');
			mainVisualStatus = false;
		}
...
defaultObj.win.on({
	...
	'ready' : function() {
		...
		if(popupId != null && $.jutil.getCookie("mainPopup_"+popupId) != 'hide') {
